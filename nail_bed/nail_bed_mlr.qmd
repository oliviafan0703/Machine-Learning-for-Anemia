...
colors <- c("y_actual" = "red", "y_predicted" = "blue")
# ggplot(df, aes(index)) +
#   labs(title="Actual vs. Predicted Concentration in LASSO Model")+
#   geom_point(aes(y=y.test), colour="red", ylab="Value") +  # first layer
#   geom_point(aes(y=lasso.pred), colour="blue")+
#   scale_color_manual(values = colors)

ggplot(df, aes(x = index)) +
    geom_point(aes(y = y.test, color = "y_actual"), size = 1.5) +
    geom_point(aes(y = lasso.pred, color = "y_predicted"), size = 1.5) +
    labs(x = "Index",
         y = "Value",
         color = "Legend") +
    scale_color_manual(values = colors)
